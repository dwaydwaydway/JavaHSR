...
public class ClientGUI extends JFrame {
	...
	public ClientGUI() {
		...
		JScrollPane visual = new JScrollPane(Available, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
									...
									btnCancel.jbutton.addActionListener(new ActionListener() {
										public void actionPerformed(ActionEvent e) {
											...
											try {
												...
												if (msg.getClass() == new Success_Message().getClass()) {
													System.out.println(
															"success return after canceling the ticket ( in bytn mode)");
		...
		btnNewButton_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				// We have to remember the date, be cause the database will only return time
				// without day
				...
				if (SearchCarMessage.getHour() < 0 || SearchCarMessage.getHour() >= 24
						|| SearchCarMessage.getMinute() >= 60 || SearchCarMessage.getMinute() < 0) {
					// do nothing at following function
					System.out.println("Invaild time setting");
					vail = false;
				}

				if ((int) departHour_re.getValue() < 0 || (int) departHour_re.getValue() >= 24
						|| (int) departMinute_re.getValue() >= 60 || (int) departMinute_re.getValue() < 0) {
					// do nothing at following function
					System.out.println("Invaild time setting");
					vail = false;
				}
									...
									for (int k = 0; k < 11; k++) {
										...
										case 1:
											temp.setText(SearchCarMessage.getDepartDay() + ","
													+ info1.carList.get(i).getDepartTime());
											break;
										case 2:
										...
										case 3:
											temp.setText(SearchCarMessage.getDepartDay() + ","
													+ info1.carList.get(i).getArriveTime());
											break;
										case 4:
				...
				// If return trip is ordered
					...
					try {
						...
						if (msg.getClass() == new Available().getClass()) {
							...
							SearchCarMessage
									.setDepartDay(datePicker_return_trip.getJFormattedTextField().getText().toString());
							System.out.println(SearchCarMessage.toString());
...
//															}
		...
		/**
		 * on the Booking History pane as we push the button "Search"
		 * 
